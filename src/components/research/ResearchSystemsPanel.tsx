          ...
          return (
            ...
            >
              <div className="flex items-center justify-between gap-2">
                <p className="font-semibold text-slate-900">{sys.label}</p>
                <span
              ...
              <p className="mt-2 text-sm text-slate-600">{sys.detail}</p>
            </div>
          );
        })}
      </div>
      <p className="mt-6 text-xs text-slate-500">
        Configure API keys in Vercel (<code className="rounded bg-slate-100 px-1">OPENAI_API_KEY</code>,{" "}
        <code className="rounded bg-slate-100 px-1">XAI_API_KEY</code>,{" "}
        <code className="rounded bg-slate-100 px-1">CORE_API_KEY</code>, etc.) for full live
