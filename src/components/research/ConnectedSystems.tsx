...
export function ConnectedSystems({ moduleId }: { moduleId: string }) {
  ...
  return (
    <div className="mt-4 rounded-lg border border-line bg-cream-50 px-4 py-3">
      ...
      <div className="mt-2 flex flex-wrap gap-1.5">
        {integrations.map((s) => (
          <span
            key={s!.id}
            className="rounded-full border border-brand-200 bg-offwhite px-2.5 py-0.5 text-xs font-medium text-brand-800"
          >
            {s!.name}
          </span>
        ))}
