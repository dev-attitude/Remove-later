        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3">
          <div className="flex min-w-0 items-center gap-3">
            <Link href={`/campus/${tenant.slug}`} className="min-w-0">
              <p className="truncate text-sm font-semibold text-charcoal">{tenant.name}</p>
              <p className="truncate text-xs text-muted">{SMARTCAMPUS.productName}</p>
            </Link>
          </div>
        ...
        >
          <div className="sticky top-6 space-y-4">
            <div className="rounded-xl border border-line bg-offwhite p-4">
              ...
              <p className="mt-1 text-sm text-charcoal">{tenant.tagline}</p>
              <p className="mt-2 text-xs text-muted">
                Role: <span className="font-medium capitalize">{role}</span>
              </p>
            </div>
            <nav className="rounded-xl border border-line bg-offwhite p-2">
              {nav.map((item, idx) => {
                ...
                return (
                  <div key={item.id}>
                    {newSection && (
                      <p className="mt-3 mb-1 px-3 text-[10px] font-bold uppercase tracking-wider text-muted first:mt-1">
                        {item.section}
                      </p>
                    )}
                    <Link
                      ...
                      {item.badge && (
                        <span
                          className={clsx(
                            ...
                            active ? "bg-offwhite/20 text-offwhite" : "bg-blue-100 text-blue-700"
                          )}
                        >
                          {item.badge}
                        </span>
  ...
  return (
    <div className="relative">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-1 rounded-lg border border-line px-3 py-1.5 text-xs font-medium text-charcoal hover:bg-cream-50"
      >
        Switch institution
        <ChevronDown className="h-3.5 w-3.5" />
      </button>
      {open && (
        <div className="absolute right-0 z-20 mt-1 w-52 rounded-lg border border-line bg-offwhite py-1 shadow-lg">
          {tenants.map((t) => (
            <Link
              key={t.slug}
              href={`/campus/${t.slug}`}
              className={clsx(
                "block px-3 py-2 text-sm hover:bg-cream-50",
                t.slug === current ? "font-semibold text-blue-700" : "text-charcoal"
              )}
