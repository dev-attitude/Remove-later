...
export function JunAccountPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4 rounded-xl border border-line bg-offwhite p-6 shadow-sm">
        <div className="flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-navy to-royal text-2xl font-bold text-offwhite">
          {initial}
        </div>
        <div>
          <p className="text-lg font-bold text-navy">{account.customer.name}</p>
          <p className="text-sm text-muted">{account.customer.email}</p>
          <p className="text-xs text-muted">Account ID · {account.orderId}</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          <Link
            key={href}
            href={href}
            className="flex items-center gap-3 rounded-xl border border-line bg-offwhite p-4 shadow-sm transition hover:border-royal/30"
          >
      ...
      <div className="rounded-xl border border-line bg-offwhite p-6 shadow-sm">
        <h2 className="font-bold text-navy">Billing summary</h2>
        <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
          <div className="flex justify-between rounded-lg bg-cream-50 p-3">
            <dt className="text-muted">Monthly services</dt>
            <dd className="font-semibold">{formatPrice(account.monthlyTotal, "/mo")}</dd>
          </div>
          <div className="flex justify-between rounded-lg bg-cream-50 p-3">
            <dt className="text-muted">Yearly renewals</dt>
        ...
        <ul className="mt-4 space-y-4">
          <li className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-muted">Extra security on login</p>
            </div>
            <ToggleSwitch
          ...
          <li className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-muted">Renewal and service alerts</p>
            </div>
            <ToggleSwitch
          ...
          <li className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-muted">Text alerts to {account.customer.phone || "your phone"}</p>
            </div>
            <ToggleSwitch
