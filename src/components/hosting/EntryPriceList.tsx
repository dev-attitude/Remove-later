...
export function StarterBundleBanner() {
  ...
  return (
    <div className="rounded-2xl bg-gradient-to-r from-navy to-royal p-8 text-offwhite md:p-10">
      <div className="flex flex-wrap items-start justify-between gap-6">
        <div className="max-w-xl">
          ...
          <p className="mt-2 text-offwhite">{STARTER_BUNDLE.audience}</p>
          <ul className="mt-4 grid gap-2 sm:grid-cols-2">
            {STARTER_BUNDLE.includes.map((item) => (
              <li key={item} className="flex gap-2 text-sm text-offwhite">
        ...
        <div className="rounded-xl bg-offwhite/10 p-6 text-center">
          <p className="text-3xl font-bold">
            {formatPrice(STARTER_BUNDLE.upfrontNad)}{" "}
            <span className="text-base font-semibold text-offwhite">upfront</span>
          </p>
          ...
          <Link
            ...
            className="mt-4 inline-block rounded-lg bg-offwhite px-5 py-2.5 text-sm font-semibold text-navy transition hover:bg-sky/90"
          >
            Get this bundle
          </Link>
        </div>
    ...
    <div>
      <div className="grid gap-8 md:grid-cols-2">
        {ENTRY_PRICE_LIST.map((cat) => (
          <section
            key={cat.id}
            className="overflow-hidden rounded-2xl border border-line bg-offwhite shadow-sm"
          >
            <h2 className="border-b border-line bg-cream-50 px-5 py-3 text-base font-bold text-navy">
              {cat.title}
            </h2>
              ...
              <tbody>
                {cat.rows.map((row) => (
                  <tr key={row.service} className="border-b border-line last:border-0">
                    <td className="px-5 py-2.5 text-charcoal">{row.service}</td>
