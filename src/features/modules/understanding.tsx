        ...
        <div className="space-y-3">
          {UNDERSTANDING_RESEARCH_TOPICS.map((mod) => {
            ...
            return (
              <div
                key={mod.module}
                className="overflow-hidden rounded-xl border border-line bg-offwhite shadow-sm"
              >
                <button
                  type="button"
                  className="flex w-full items-center justify-between gap-3 px-4 py-3.5 text-left hover:bg-cream-50"
                  onClick={() => setExpandedModule(moduleOpen ? null : mod.module)}
                >
                  <span className="font-display text-sm font-semibold text-charcoal sm:text-base">
                    {mod.module}
                  </span>
                  {moduleOpen ? (
                      ...
                      return (
                        <li key={item} className="border-b border-line last:border-b-0">
                          ...
                          {active && (
                            <div
                              ref={contentRef}
                              className="border-t border-brand-100 bg-cream-50/60 px-4 py-6 sm:px-6 sm:py-8 lg:px-10 lg:py-10"
                            >
                              <TopicContent
                                selected={selected}
                                content={content}
  ...
  return (
    <div className="space-y-6">
      {error && (
          ...
          {error.includes("trial") && (
            ...
            >
              View subscription plans
                ...
                <div className="overflow-hidden rounded-2xl border border-line bg-offwhite shadow-md">
        <div className="border-b border-line bg-gradient-to-br from-cream-50 via-offwhite to-brand-50/50 px-5 py-6 sm:px-8 sm:py-8">
          <p className="text-xs font-semibold uppercase tracking-widest text-brand-700">
            {selected.module}
          </p>
          <h2 className="mt-2 font-display text-2xl font-bold leading-tight text-charcoal sm:text-3xl">
            {selected.label}
          </h2>
        ...
        <section className="border-t border-line bg-cream-50/40 px-5 py-8 sm:px-8 sm:py-10 lg:px-10 lg:py-12">
...
function ReferenceCard({
  ...
  return (
    <li className="rounded-xl border border-line bg-offwhite p-5 sm:p-6">
      ...
      <p className="mt-2 text-lg font-semibold leading-snug text-charcoal sm:text-xl">
        {refItem.title}
      </p>
      ...
      {refItem.doi && (
        <p className="mt-2 font-mono text-xs text-muted">DOI: {refItem.doi}</p>
      )}
      {refItem.excerpt ? (
        <p className="mt-4 text-base leading-[1.75] text-charcoal">{refItem.excerpt}</p>
      ) : (
        <p className="mt-4 text-sm italic text-muted">No excerpt available.</p>
      )}
    </li>
  );
}
