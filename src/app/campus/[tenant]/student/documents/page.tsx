...
export default async function DocumentsPage({ params }: Props) {
  ...
  return (
    <div>
          ...
          return (
            ...
            >
              <div className="flex items-start gap-3">
                <QrCode className="mt-0.5 h-5 w-5 text-muted" />
                <div>
                  <p className="font-medium text-charcoal">{d.name}</p>
                  <p className="text-xs text-muted">{d.description}</p>
                  {available && <p className="text-xs text-muted">Updated {d.updated}</p>}
                </div>
              </div>
              <button
                type="button"
                disabled={!available}
                className="inline-flex items-center gap-1.5 rounded-lg bg-charcoal px-3 py-2 text-xs font-medium text-offwhite disabled:cursor-not-allowed disabled:opacity-40"
              >
      ...
      <h2 className="mb-3 mt-10 text-lg font-semibold text-charcoal">Digital Certificate Vault</h2>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {CERTIFICATE_VAULT.map((c) => (
          <div key={c.name} className="rounded-xl border border-line bg-offwhite p-4">
            <Award className="h-6 w-6 text-amber-500" />
            <p className="mt-2 font-medium text-charcoal">{c.name}</p>
