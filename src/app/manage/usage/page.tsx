...
export default function ManageUsagePage() {
  ...
  if (!data) return <p className="text-sm text-muted">Loading usage analytics…</p>;
  ...
  return (
    <div>
      ...
      <div className="mb-8 grid gap-4 sm:grid-cols-2">
        <Card className="!p-4">
          <p className="text-xs text-muted">Actions (last 30 days)</p>
          <p className="mt-2 text-2xl font-bold text-brand-700">{data.last30}</p>
        </Card>
        <Card className="!p-4">
          <p className="text-xs text-muted">Actions (all time)</p>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
              ...
              {data.byAction.map((a) => (
                <li key={a.action}>
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-semibold text-charcoal">{a.count}</span>
                  </div>
        ...
        <Card>
          ...
          {data.topUsers.length === 0 ? (
            <p className="mt-4 text-sm text-muted">No signed-in activity yet.</p>
          ) : (
            ...
            <tbody>
              {data.recent.map((log) => (
                <tr key={log.id} className="border-b border-line last:border-0">
                  <td className="px-4 py-2.5 text-muted">{fmtDateTime(log.createdAt)}</td>
                  ...
                  <td className="px-4 py-2.5 text-muted">{log.portal ?? "—"}</td>
                </tr>
              ))}
